...
int shift_move(int production_number)
{
  ...
  while (token != NULL)
  {
    if (strcmp("->", token) == 0)
      flag = 1;
    if (flag)
      strcpy(temp_stck[temp_top++], token);
    token = strtok(NULL, " \n\t");
  }

  while (temp_top > 0)
  {
    if (strcmp("->", temp_stck[temp_top - 1]) != 0)
      push(temp_stck[temp_top - 1]);
    temp_top--;
  }
  return 0;
}
...
int main()
{
  ...
  {
    print_contents();
    strcpy(stck_top, stck[top - 1]);
    int is_nt = check_nt(stck_top);

    // Top of stck is Non Terminal
    if (is_nt)
    {
      int ter_no = find_terminal(tokens[curr_trow]);
      int non_ter_no = find_non_terminal(stck_top);
    ...
    // Top of stck is Terminal
    else
    {
      // Terminal - terminal match , therefore pop()
      if (tokens[curr_trow] == stck_top)
      {
        printf("\nMATCH!\n");
        pop();
      }
